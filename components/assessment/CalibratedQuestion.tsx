  ...
  return (
    <div className="bg-white shadow-xl rounded-2xl overflow-hidden max-w-2xl mx-auto border border-gray-100 my-8">
      <div className="bg-slate-50 p-6 border-b border-gray-100 flex justify-between items-center">
        <div>
           <h3 className="text-xs font-bold text-indigo-600 uppercase tracking-wider">Skill Assessment</h3>
        ...
        {step === 'self-rate' && (
          <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="space-y-4">
                ...
                />
                <div className="flex justify-center text-4xl font-bold text-indigo-600">
                    {selfRating}<span className="text-xl text-gray-400 font-medium pt-3">/5</span>
                </div>
            </div>
            <button onClick={handleSelfRate} className="w-full py-4 bg-gray-900 text-white rounded-xl font-bold hover:bg-black transition-all">
                Start Challenge
            </button>
          </div>
        )}
          ...
          <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="grid gap-3">
                {Object.entries(questionData.options).map(([key, val]) => {
                    ...
                    return (
                        <button key={key} disabled={!!selectedOption} onClick={() => handleSubmit(key)} className={`p-4 text-left border-2 rounded-xl transition-all duration-200 ${borderClass}`}>
